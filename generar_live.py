...
def main():
    ...
    if ok:
        # Contar hoteles únicos para el mensaje
        hoteles = []
        for r in rows:
            h = r.get("hotel")
            if h and h not in hoteles:
                hoteles.append(h)
        print(f"OK -> live.html actualizado (hoteles={len(hoteles)}, sustituciones_csv={len(sustituciones)})")

...
output_html = "cuadrante.html"
ignore_sheets = ["Sustituciones", "Hoja1", "Datos de Validación"]

ABSENCE_COLOR = {"vacaciones":"#FF4C4C","baja":"#A64CA6","permiso":"#4C6AFF","formacion":"#FFA64C","festivo":"#4CAF50","libranza":"#B59F3B"}

def _strip(s): return "" if s is None else str(s).strip()
def _canon(s): import unicodedata, re; s=_strip(s); s2=''.join(c for c in unicodedata.normalize('NFD', s) if unicodedata.category(c)!='Mn').lower(); return re.sub(r'\s+',' ',s2)
...
def process_excel_sheets():
    ...
    if not os.path.exists(excel_path): print(f"❌ Falta Excel: {excel_path}"); return
    if not os.path.exists(template_path): print(f"❌ Falta plantilla: {template_path}"); return
    ...
    for sh in xls.sheet_names:
        ...
        for e in df.get("Empleado", pd.Series(dtype=str)).astype(str).fillna(""):
            e=e.strip()
            if e and e not in order_map: order_map[e]=order; order+=1
        ...
        if "Semana" in df.columns and "Empleado" in df.columns:
            for sem, dfx in df.groupby("Semana", dropna=False):
                ...
                for e in dfx["Empleado"].astype(str).map(str.strip):
                    if e and e not in ordmap: ordmap[e]=idx; idx+=1
                order_week_map[(sh, sem_key)]=ordmap
        hoteles.append(df)
    if not hoteles: print("❌ No hay hojas de hoteles"); return
    df=pd.concat(hoteles, ignore_index=True)
    df.columns=df.columns.str.strip()
    if "Semana" not in df.columns: print("❌ Falta columna 'Semana' en el Excel"); return
    ...
    melted=df.melt(id_vars=["Semana","Empleado","Hotel","OrderBaseHotel"], value_vars=days, var_name="Dia", value_name="TurnoRaw").dropna(subset=["Empleado"]).copy()
    ...
    if "Sustituciones" in xls.sheet_names:
        ...
        if not s0.empty:
            ...
            s0=s0.rename(columns={"Hotel":"Hotel","Empleado":"Empleado","Fecha":"Fecha","Sustituto":"Sustituto","Tipo Ausencia":"TipoAusencia","Cambio de Turno":"CambioDeTurno"})
    ...
    agg=melted.groupby(["Hotel","Semana","Empleado"], as_index=False).agg(BaseSemana=("EmpleadoOrdenSemanaBase","min"), DiaMin=("OrderDia","min"), VacSemana=("VacFlag","max"))

...
def classify_cell(val: str):
    ...
    if ak and ak != "descanso":
        return {"code":s, "long":s, "is_abs":True, "abs_key":ak}   # etiqueta EXACTA tal cual Excel
    if ak == "descanso":
        return {"code":"D", "long":"Descanso", "is_abs":False, "abs_key":""}

    # Turnos por palabras habituales
    ...
    # Si no clasificamos, mantenemos el texto literal como 'long'
...
def process_excel_sheets():
    ...
    for sh in xls.sheet_names:
        ...
        df["Hotel"] = sh
        # Orden base por primera aparición del empleado en la hoja (columna B)
        order_map = {}
        order = 0
        ...
        # orden por semana (aparición exacta esa semana)
    ...
    # TextoDia = lo que se debe mostrar/exportar EXACTO
    melted["TextoDia"] = melted["TurnoLargo"].fillna("").astype(str)

    melted["TipoEmpleado"] = ["Normal"]*len(melted)
    melted["NameColorC"] = [""]*len(melted)
    melted["Icono"] = [""]*len(melted)  # 🔄 solo para cambio de turno
    melted["Sustituto"] = [""]*len(melted)
    melted["SustitucionPor"] = [""]*len(melted)
    ...
    # Copia antes de ausencias para recuperar turno original del titular
    melted_orig = melted.copy()

    # Marcar ausencias de celdas con su texto exacto
    for i, c in enumerate(clist):
        if c["is_abs"] and c["abs_key"]:
            ...
            melted.loc[i, "TextoDia"] = melted.loc[i,"TurnoLargo"]  # exacto
    ...
    for _, r in sust.iterrows():
        ...
        # Ausencia desde hoja Sustituciones → usar texto EXACTO de F
        tipo_exact = _strip(tipo_raw)
        if tipo_exact and not idx_emp.empty:
            ak = is_absence_text(_canon(tipo_exact))
            if ak:
                melted.loc[idx_emp, ["Turno","TurnoLargo","NameColorC","Icono","TipoEmpleado","SustitucionPor","TextoDia"]] =                     [tipo_exact, tipo_exact, ABSENCE_COLOR.get(ak, "#FF4C4C"), "", "Ausente", tipo_exact, tipo_exact]

        # Sustituto: hereda turno del titular y su posición ese día
